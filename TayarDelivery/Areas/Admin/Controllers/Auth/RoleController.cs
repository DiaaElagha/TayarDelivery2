...
namespace TayarDelivery.Areas.Admin.Controllers.Auth
{
    ...
    public class RoleController : BaseController
    {
        ...
        public JsonResult AjaxData([FromBody] dynamic data)
        {
            ...
            var query = _roleRepository.Filter(
        ...
        public async Task<IActionResult> Create(RoleVM model)
        {
            ...
            {
                var newEntity = _mapper.Map<Role>(model);
                var result = await _roleRepository.AddAsync(newEntity);
                if (result != null)
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            var model = await _roleRepository.Get(id);
            if (model == null)
            {
                return NotFound();
            }
            var newEntity = _mapper.Map<RoleVM>(model);
        ...
        public async Task<IActionResult> Edit(int id, RoleVM model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    var baseEntoty = await _roleRepository.Get(id);
                    PropertyCopy.Copy(model, baseEntoty);
                    await _roleRepository.UpdateAsync(baseEntoty);
                }
        ...
        public async Task<IActionResult> Delete(int? id)
        {
            ...
            var result = await _roleRepository.DeleteAsync(model);
            if (result != null)
        ...
        {
            return await _roleRepository.Any(e => e.Id == id);
        }
    }
}
